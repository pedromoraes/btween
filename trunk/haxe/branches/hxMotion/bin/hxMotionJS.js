...
hxmotion.shortcuts.HxM.tween = function(target,params) {
	$s.push("hxmotion.shortcuts.HxM::tween");
	var $spos = $s.length;
...
hxmotion.shortcuts.HxM.delay = function(interval) {
	$s.push("hxmotion.shortcuts.HxM::delay");
	var $spos = $s.length;
	{
...
hxmotion.Delay.prototype.onEnterFrame = function(evt) {
	$s.push("hxmotion.Delay::onEnterFrame");
	var $spos = $s.length;
...
jsflash.display.Sprite.prototype.dispatch = function() {
	$s.push("jsflash.display.Sprite::dispatch");
	var $spos = $s.length;
...
hxmotion.BTween.prototype.consume = function(args) {
	...
	if(args != null) {
		...
		if(Reflect.hasField(args,"mod")) {
			this.modifierArgs = [{ }];
			if(!Std["is"](args.mod,Array)) this.modifier = args.mod;
			else {
				this.modifier = args.mod.shift();
				this.modifierArgs = this.modifierArgs.concat(args.mod);
			}
...
hxmotion.modifiers.CSS.setPos = function(ix,round,pers,target,prop,value,unit) {
	...
	if(pers.initValue != null) initValue = pers.initValue;
		...
		if(prop1 == null) initValue = pers.initValue = 0;
		else initValue = pers.initValue = Std.parseInt(prop1.split(unit).join(""));
	}
	if(initValue == null) initValue = pers.initValue = 0;
	var delta = value - initValue;
	target[prop] = Std["int"](initValue + delta * ix) + unit;
	$s.pop();
}
