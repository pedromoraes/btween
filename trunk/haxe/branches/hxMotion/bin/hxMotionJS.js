...
hxmotion.shortcuts.HxM.delay = function(interval) {
...
MainJS.init = function(e) {
	...
	hxmotion.shortcuts.HxM.tween(div.style,{ time : 300, modifier : $closure(hxmotion.modifiers.CSSPos,"byPixels"), args : ["top",300]}).start().queue(hxmotion.shortcuts.HxM.tween(div.style,{ time : 300, modifier : $closure(hxmotion.modifiers.CSSPos,"byPixels"), args : ["top",100]})).queue($closure(haxe.Log,"trace"),"done, callback");
}
...
Lambda.count = function(it) {
	...
	while( $it14.hasNext() ) { var _ = $it14.next();
	++n;
	}}
	return n;
}
...
jsflash.display.Sprite.__super__ = neash.events.EventDispatcher;
for(var k in neash.events.EventDispatcher.prototype ) jsflash.display.Sprite.prototype[k] = neash.events.EventDispatcher.prototype[k];
jsflash.display.Sprite.prototype.dispatch = function() {
...
hxmotion.BTween.prototype.consume = function(args) {
	if(args != null) {
		...
		if(Reflect.hasField(args,"modifier")) {
			this.modifier = args.modifier;
			Reflect.deleteField(args,"modifier");
			this.modifierArgs = [{ }];
			if(Reflect.hasField(args,"args")) {
				this.modifierArgs = args.args.concat(this.modifierArgs);
				Reflect.deleteField(args,"args");
			}
		}
		{
...
hxmotion.BTween.prototype.start = function(args) {
	...
	this.startTime = jsflash.Lib.getTimer();
	{
		var _g = 0, _g1 = this.props;
		while(_g < _g1.length) {
			var prop = _g1[_g];
...
hxmotion.BTween.prototype.step = function(evt) {
	var t = jsflash.Lib.getTimer() - this.startTime;
	var index;
	var finished = false;
	if(t >= this.time) {
		index = 1.0;
	...
	if(this.modifier != null) {
		var stepArgs = [index,this.rounded,this.target];
		if(this.modifierArgs != null) stepArgs = stepArgs.concat(this.modifierArgs);
		this.modifier.apply(null,stepArgs);
	}
	{
		var _g = 0, _g1 = this.props;
		while(_g < _g1.length) {
...
hxmotion.BTween.prototype.toString = function() {
	...
	return ("BTween: target:" + this.target + ",time=" + this.time + ",props:" + propsList + " modifier:" + this.modifier + ",modifierArgs:" + this.modifierArgs);
}
hxmotion.BTween.prototype.update = function(listener) {
	if(this.updateListeners == null) this.updateListeners = [];
...
hxmotion.modifiers = {}
hxmotion.modifiers.CSSPos = function() { }
hxmotion.modifiers.CSSPos.__name__ = ["hxmotion","modifiers","CSSPos"];
hxmotion.modifiers.CSSPos.byPixels = function(ix,round,target,prop,value,obj) {
	var initValue;
	if(obj.initValue != null) initValue = obj.initValue;
	else {
		var prop1 = Reflect.field(target,prop);
		if(prop1 == null) initValue = obj.initValue = 0;
		else initValue = obj.initValue = Std.parseInt(prop1.split("px").join(""));
	}
	if(initValue == null) initValue = obj.initValue = 0;
	var delta = value - initValue;
	target[prop] = Std["int"](initValue + delta * ix) + "px";
}
hxmotion.modifiers.CSSPos.prototype.__class__ = hxmotion.modifiers.CSSPos;
...
hxmotion.BTween.modifierProp = "modifier";
hxmotion.BTween.argsProp = "args";
hxmotion.BTween.roundedProp = "rounded";
hxmotion.BTween.targetProp = "target";
hxmotion.BTween.enterFrameDispatcher = new jsflash.display.Sprite();
js.Lib.onerror = null;
